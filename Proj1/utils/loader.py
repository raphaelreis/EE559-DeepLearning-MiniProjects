...
class PairSetMNIST(Dataset):
    ...
    Data can be augmented in three ways: 
        ...
        2) transaltion one pixel upward,downward,left and right
    ...
    def __init__(self,rotate,translate,swap_channel) :
        ...
        #Data augmentation
        #train_input_rotated = torch.empty(train_input.shape)
        if (rotate == True) :
        ...
        #train_input_translated = torch.empty(train_input.shape)    
        if (translate == True) :
